...
var be = (e, a, t) => a in e ?
    Pt(e, a, {enumerable: !0, configurable: !0, writable: !0, value: t}) :
    e[a] = t,
    Ee = (e, a) => {
      for (var t in a || (a = {})) Nt.call(a, t) && be(e, t, a[t]);
      if (Ue)
        for (var t of Ue(a)) yt.call(a, t) && be(e, t, a[t]);
      return e
    };
import {V as w, aq as Bt, aD as ze, x as Rt, R as Ut, w as bt, ar as Et, H as zt, F as Xe, as as Xt, _ as wt, O as he, af as we, d as Se, P as Dt, al as Vt} from './vendor.b603f603.js';
import {k as Ct, G as De, S as Me, T as Ve, n as j, D as Ot, t as ae, o as $, p as D, q as Qt, r as Yt, v as Ht, w as ie, x as pe, y as Wt, z as kt, A as qt, B as Fe} from './utils.684b27f5.js';
import {D as Ce} from './DisneyMaterial.dae81f5f.js';
const Oe = ['EXT_color_buffer_float', 'EXT_float_blend'],
      Zt = ['OES_texture_float_linear'];
var jt = {
  source:
      'layout(location=0)in vec2 a_position;out vec2 vCoord;void main(){vCoord=a_position;gl_Position=vec4(2.*a_position-1.,0,1);}'
};
function Kt(e) {
  const a = e.createVertexArray();
  e.bindVertexArray(a), e.bindBuffer(e.ARRAY_BUFFER, e.createBuffer()),
      e.bufferData(
          e.ARRAY_BUFFER,
  ...
  const i = Ct(e, {vertex: jt});
  function o() {
    e.bindVertexArray(a), e.drawArrays(e.TRIANGLES, 0, 6)
  }
  return {
    draw: o, vertexShader: i
  }
}
const Jt = 0, $t = 1, ea = 2, ta = 3, aa = 4;
function ia(...e) {
  let a = 0;
  for (let i = 0; i < e.length; i++) {
    const o = e[i], r = o.data ? o.data.length / o.channels : 0;
    a = Math.max(a, r)
  }
  const t = [];
  for (let i = 0; i < a; i++)
    for (let o = 0; o < e.length; o++) {
      const {data: r = [], channels: l} = e[o];
      for (let s = 0; s < l; s++) t.push(r[i * l + s])
    }
  return t
}
function na(e) {
  const a = {};
  return a.position = e.map(t => t.position),
         a.emission = e.map(t => t.emission), a.p1 = e.map(t => t.p1),
         a.p2 = e.map(t => t.p2), a.radius = e.map(t => t.radius),
         a.area = e.map(t => t.area), a.type = e.map(t => t.type),
         a.visible = e.map(t => t.visible),
         a.position = [].concat(...a.position.map(t => t.toArray())),
         a.emission = [].concat(...a.emission.map(t => t.toArray())),
         a.p1 = [].concat(...a.p1.map(t => t.toArray())),
         a.p2 = [].concat(...a.p2.map(t => t.toArray())),
         a.params =
             ia({data: a.radius, channels: 1}, {data: a.area, channels: 1},
                {data: a.type, channels: 1}, {data: a.visible, channels: 1}),
         a
}
function oa(e) {
  const a = e.map(t => {
    const i = {};
    switch (i.position = t.position,
            i.emission = t.color.multiplyScalar(t.intensity),
            i.radius = t.radius || 0, i.area = 0, i.visible = Number(t.visible),
            i.p1 = new w, i.p2 = new w, t.type) {
      case 'RectAreaLight':
        if (i.type = Jt, t.width && t.height) {
          const o = new Bt(t.width, t.height), r = new w;
          t.target && r.copy(t.target);
          const l = new w().subVectors(t.position, r),
                s = new w().copy(l).negate();
          o.lookAt(s);
          const n = o.attributes.position.array,
                f = new w(n[0], n[1], n[2]).add(t.position);
          new w(n[3], n[4], n[5]).add(t.position);
          const u = new w(n[6], n[7], n[8]).add(t.position),
                p = new w(n[9], n[10], n[11]).add(t.position);
          i.position.copy(u), i.p1 = p.sub(u), i.p2 = f.sub(u),
                              i.area = new w().crossVectors(i.p1, i.p2).length()
        }
        break;
      case 'QuadLight':
        i.type = $t, i.p1 = t.v1.sub(t.position), i.p2 = t.v2.sub(t.position),
        i.area = new w().crossVectors(i.p1, i.p2).length();
        break;
      case 'SphereAreaLight':
        i.type = ea, i.area = 4 * Math.PI * t.radius * t.radius;
        break;
      case 'PointLight':
        i.type = aa, i.area = 0;
        break;
      case 'DirectionalLight':
        i.type = ta, t.target && i.p1.copy(t.target), i.area = 0;
        break;
      default:
        console.warn(`Not support light type: ${t.type}`);
        break
    }
    return i
  });
...
function ne(e, a) {
  ...
  e.traverse(u => {
    u.isMesh &&
        (u.geometry ?
             u.material ?
             (u.material.isMeshStandardMaterial ?
                  u.material = new Ce().fromStandardMaterial(u.material) :
                  u.material.isRayTracingMaterial ||
                      (u.material = new Ce().fromBasicMaterial(u.material)),
              t.push(u)) :
             console.warn(u, 'must have a material property.') :
             console.warn(u, 'must have a geometry property')),
    u.isLight && i.push(u)
  });
  const o = {data: e.environment, intensity: e.envMapIntensity || 1},
        r = o.data && o.data.isTexture, l = i.length || 0;
  let s = null;
  l && (s = oa(i));
  const n = new Map;
  for (const u of t) {
    const p = u.material;
    let L = n.get(p);
    L === void 0 && (L = n.size, n.set(p, L))
  }
  const f = De(n).bufferSize;
  return {
    environment: o, isTextureEnv: r, camera: a, meshes: t, meshLights: s,
        meshLightsNum: l, materialIndexMap: n, materials: Array.from(n.keys()),
        uvTransformBufferDataLength: f
  }
}
function Qe(e, a) {
  const t = a.materialIndexMap, {
    mapNum: i,
    perMatrixLength: o,
    perWrappingDataLength: r,
    uvTransDataLength: l,
    bufferSize: s
  } = De(t);
  let n = new Float32Array(s);
  const f = new ze, u = i * o, p = i * r;
  t.forEach((m, x) => {Me.forEach((S, I) => {
              const _ = x[`${S}`];
              let h = f.elements;
              _ && (_.updateMatrix(), h = _.matrix.elements);
              for (let G = 0; G < o; G++) n[u * m + I * o + G] = h[G];
              let T = 0, v = 0;
              _ && (T = Ve.get(_.wrapS), v = Ve.get(_.wrapT)),
                  n[l + m * p + I * r + 0] = T, n[l + m * p + I * r + 1] = v,
                                        n[l + m * p + I * r + 2] = 0
            })});
  const L = j(e, n, 3);
  function d() {
  ...
  return {
    uvTransformBufferTex: L, uvTransformBufferDataCount: n.length / 3,
        wrappingRootIndex: l / 3, dispose: d
  }
}
var ra = `uniform sampler2D uvTransformBuffer;
...
function sa(e, a, t) {
  const i = e.getActiveUniformBlockParameter(
      a, t, e.UNIFORM_BLOCK_ACTIVE_UNIFORM_INDICES),
        o = e.getActiveUniforms(a, i, e.UNIFORM_OFFSET),
        r = e.getActiveUniforms(a, i, e.UNIFORM_ARRAY_STRIDE), l = {};
  for (let s = 0; s < i.length; s++) {
    const {name: n, type: f, size: u} = e.getActiveUniform(a, i[s]);
    l[n] = { type: f, size: u, offset: o[s], stride: r[s] }
  }
  return l
}
function O(e, a, t, i, o, r, l) {
  const s = Math.min(l.length / r, t);
  for (let n = 0; n < s; n++)
    for (let f = 0; f < r; f++) e[a](i + n * o + f * 4, l[r * n + f], !0)
}
function la(e, a, t) {
  const i = e.getUniformBlockIndex(a, t),
        o = e.getActiveUniformBlockParameter(a, i, e.UNIFORM_BLOCK_DATA_SIZE),
        r = sa(e, a, i), l = e.createBuffer();
  e.bindBuffer(e.UNIFORM_BUFFER, l),
      e.bufferData(e.UNIFORM_BUFFER, o, e.STATIC_DRAW);
  const s = new DataView(new ArrayBuffer(o));
  function n(p, L) {
    if (!r[p]) return;
    const {type: d, size: m, offset: x, stride: S} = r[p];
    switch (d) {
      case e.FLOAT:
        O(s, 'setFloat32', m, x, S, 1, L);
        break;
      case e.FLOAT_VEC2:
        O(s, 'setFloat32', m, x, S, 2, L);
        break;
      case e.FLOAT_VEC3:
        O(s, 'setFloat32', m, x, S, 3, L);
        break;
      case e.FLOAT_VEC4:
        O(s, 'setFloat32', m, x, S, 4, L);
        break;
      case e.INT:
        O(s, 'setInt32', m, x, S, 1, L);
        break;
      case e.INT_VEC2:
        O(s, 'setInt32', m, x, S, 2, L);
        break;
      case e.INT_VEC3:
        O(s, 'setInt32', m, x, S, 3, L);
        break;
      case e.INT_VEC4:
        O(s, 'setInt32', m, x, S, 4, L);
        break;
      case e.BOOL:
        O(s, 'setUint32', m, x, S, 1, L);
        break;
      default:
  ...
  function f(p) {
    e.bindBuffer(e.UNIFORM_BUFFER, l), e.bufferSubData(e.UNIFORM_BUFFER, 0, s),
        e.bindBufferBase(e.UNIFORM_BUFFER, p, l)
  }
  function u() {
    e.deleteBuffer(l)
  }
  return {
    set: n, bind: f, dispose: u
  }
}
function fa(e, a) {
  const t = {};
  for (const i of a) {
    const o = [];
    t[i] = { indices: He(e, i, o), textures: o }
  }
  return t
}
function Ye(e, a) {
  const t = {textures: [], indices: {}};
  for (const i of a) t.indices[i] = He(e, i, t.textures);
  return t
}
function He(e, a, t) {
  const i = [];
  for (const o of e)
    if (!(o[a] && o[a].image))
      ...
      l === t.length && t.push(o[a]), i.push(l)
    }
  return i
}
function ua(e) {
  ...
  for (const i of e)
    a.width = Math.max(a.width, i.width),
    a.height = Math.max(a.height, i.height);
  const t = [];
  for (const i of e) t.push(i.width / a.width), t.push(i.height / a.height);
  return {
    maxSize: a, relativeSizes: t
  }
}
function C(...e) {
  let a = 0;
...
function da(e, a, t) {
  const i = la(e, a, 'Materials');
  return i.set(
             'Materials.colorAndMaterialType[0]',
             C({data: [].concat(...t.color.map(o => o.toArray())), channels: 3},
               {data: t.type, channels: 1})),
         i.set(
             'Materials.roughnessMetalnessNormalScale[0]',
             C({data: t.roughness, channels: 1},
               {data: t.metalness, channels: 1}, {
                 data: [].concat(...t.normalScale.map(o => o.toArray())),
                 channels: 2
               })),
         i.set(
             'Materials.alphaSpecularTintSheenSheenTint[0]',
             C({data: t.alpha, channels: 1},
               {data: t.specularTint, channels: 1},
               {data: t.sheen, channels: 1}, {data: t.sheenTint, channels: 1})),
         i.set(
             'Materials.clearcoaRoughnessSubfaceTransmission[0]',
             C({data: t.clearcoat, channels: 1},
               {data: t.clearcoatRoughness, channels: 1},
               {data: t.subsurface, channels: 1},
               {data: t.transmission, channels: 1})),
         i.set(
             'Materials.iorAtDistanceAnisotropicWorkflow[0]',
             C({data: t.ior, channels: 1}, {data: t.atDistance, channels: 1},
               {data: t.anisotropic, channels: 1},
               {data: t.workflow, channels: 1})),
         i.set(
             'Materials.specularColorGlossiness[0]',
             C({
               data: [].concat(...t.specularColor.map(o => o.toArray())),
               channels: 3
             },
               {data: t.glossiness, channels: 1})),
         i.set(
             'Materials.extinction[0]',
             C({
               data: [].concat(...t.extinction.map(o => o.toArray())),
               channels: 3
             },
               {data: t.anisotropic, channels: 1})),
         i.set(
             'Materials.diffuseNormalRoughnessMetalnessMapIndex[0]',
             C({data: t.diffuseMapIndex, channels: 1},
               {data: t.normalMapIndex, channels: 1},
               {data: t.roughnessMapIndex, channels: 1},
               {data: t.metalnessMapIndex, channels: 1})),
         i.set(
             'Materials.emissiveSpecularGlossinessMapIndex[0]',
             C({data: t.emissiveMapIndex, channels: 1},
               {data: t.specularMapIndex, channels: 1},
               {data: t.glossinessMapIndex, channels: 1},
               {data: t.emissiveMapIndex, channels: 1})),
         i.set(
             'Materials.diffuseNormalMapSize[0]',
             C({data: t.diffuseMapSize, channels: 2},
               {data: t.normalMapSize, channels: 2})),
         i.set('Materials.pbrMapSize[0]', t.pbrMapSize), i.bind(0), i
}
function We(e, a) {
  let t;
  switch (a) {
    ...
    case Ut:
      ...
      break;
    case Rt:
      t = e.GL_MIRRORED_REPEAT;
      break
  }
  return t
}
function oe(e, a, t = !1, i = 3) {
  const o = a.map(p => p.image), r = a.map(p => p.flipY),
        l = a.map(p => We(e, p.wrapS)), s = a.map(p => We(e, p.wrapT)),
        {maxSize: n, relativeSizes: f} = ua(o);
  return {
    texture: D(e, {
      width: n.width,
      height: n.height,
...
function ke(e, a) {
  const {
    materials: t,
    uvTransformBufferDataLength: i
  } = a,
 o = fa(t, ['map', 'normalMap', 'emissiveMap']),
 r = Ye(t, ['roughnessMap', 'metalnessMap']),
 l = Ye(t, ['specularMap', 'glossinessMap']), s = {}, n = {};
  if (n.color = t.map(d => d.color), n.roughness = t.map(d => d.roughness),
      n.metalness = t.map(d => d.metalness),
      n.normalScale = t.map(d => d.normalScale),
      n.specularTint = t.map(d => d.specularTint),
      n.sheen = t.map(d => d.sheen), n.sheenTint = t.map(d => d.sheenTint),
      n.clearcoat = t.map(d => d.clearcoat),
      n.clearcoatRoughness = t.map(d => d.clearcoatRoughness),
      n.transmission = t.map(d => d.transmission),
      n.subsurface = t.map(d => d.subsurface), n.ior = t.map(d => d.ior),
      n.atDistance = t.map(d => d.atDistance),
      n.extinction = t.map(d => d.extinction), n.alpha = t.map(d => d.alpha),
      n.workflow = t.map(d => d.workflow === 'Metalness' ? 0 : 1),
      n.specularColor = t.map(d => d.specularColor),
      n.glossiness = t.map(d => d.glossiness), n.type = t.map(d => Ot),
      o.map.textures.length > 0) {
    const {relativeSizes: d, texture: m} = oe(e, o.map.textures, !0, 4);
    s.diffuseMap = m, n.diffuseMapSize = d, n.diffuseMapIndex = o.map.indices
  }
  if (o.normalMap.textures.length > 0) {
    const {relativeSizes: d, texture: m} = oe(e, o.normalMap.textures, !1);
    s.normalMap = m, n.normalMapSize = d, n.normalMapIndex = o.normalMap.indices
  }
  if (r.textures.length > 0) {
    const {relativeSizes: d, texture: m} = oe(e, r.textures, !1);
    s.pbrMap = m, n.pbrMapSize = d,
    n.roughnessMapIndex = r.indices.roughnessMap,
    n.metalnessMapIndex = r.indices.metalnessMap
  }
  if (l.textures.length > 0) {
    const {relativeSizes: d, texture: m} = oe(e, l.textures, !1, 4);
    s.pbrSGMap = m, n.pbrMapSize = d,
    n.specularMapIndex = l.indices.specularMap,
    n.glossinessMapIndex = l.indices.glossinessMap
  }
  if (o.emissiveMap.textures.length > 0) {
    const {relativeSizes: d, texture: m} = oe(e, o.emissiveMap.textures, !0);
    s.emissiveMap = m, n.pbrMapSize || (n.pbrMapSize = d),
    n.emissiveMapIndex = o.emissiveMap.indices
  }
  const f = {
    NUM_MATERIALS: t.length,
    NUM_DIFFUSE_MAPS: o.map.textures.length,
    NUM_NORMAL_MAPS: o.normalMap.textures.length,
    NUM_DIFFUSE_NORMAL_MAPS:
        Math.max(o.map.textures.length, o.normalMap.textures.length),
    NUM_PBR_MAPS: r.textures.length,
    NUM_PBR_SG_MAPS: l.textures.length,
    NUM_EMISSIVE_MAPS: o.emissiveMap.textures.length,
    UVTRANSFORM_COLUMNS: ae(i / 3).columnsLog,
    SUPPORTED_MAPS: Me.length
  },
        u = $(e, {
          vertex: {source: 'void main() {}'},
          fragment: {includes: [ra], source: 'void main() {}'},
          defines: f
        }),
        p = da(e, u.program, n);
  function L() {
  ...
  return {
    defines: f, textures: s, dispose: L
  }
}
function ca(e, a = 1) {
  const t = e.length / 4, i = new Float32Array(t * 3), o = [];
  for (let r = 0; r < 255; r++) o[r] = a * Math.pow(2, r - 128) / 255;
  for (let r = 0; r < t; r++) {
    const l = e[4 * r], s = e[4 * r + 1], n = e[4 * r + 2], f = e[4 * r + 3],
          u = o[f];
    i[3 * r] = l * u, i[3 * r + 1] = s * u, i[3 * r + 2] = n * u
  }
  return i
}
function pa(e) {
  const a = document.createElement('canvas'), t = a.getContext('2d');
  return a.width = e.width, a.height = e.height, t.drawImage(e, 0, 0),
         t.getImageData(0, 0, a.width, a.height).data
}
function La(e) {
  let a;
  if (a = {
      ...
      e.data.type === Et ?
          a.data ? a.data = ca(a.data, e.intensity) :
                   (a.data = pa(e.data.image), a.dataFormat = 'byte') :
          e.data.type == zt ?
          console.error(
          ...
          e.data.type !== Xe &&
                  console.error(`No support environmentLight's data type: ${
                      e.data.type.toString()}`),
      e.data.type === Xe && e.data.format === Xt) {
    const t = e.data.image.data, i = t.length / 4, o = new Float32Array(i * 3);
    for (let r = 0; r < i; r++)
      o[r * 3 + 0] = t[r * 4 + 0], o[r * 3 + 1] = t[r * 4 + 1],
                o[r * 3 + 2] = t[r * 4 + 2];
    a.data = o
  }
  return a
...
function ma(e) {
  let a;
  return e && e.data && e.data.isTexture ?
             a = La(e) :
             console.warn(`No support environment type: ${e.data}`),
             a
}
function xa(e, a, t) {
  const i = new Float32Array(t * e * a);
  return {
    set(o, r, l, s) {
      i[t * (r * e + o) + l] = s
    }
    ...
    , width: e, height: a, channels: t, array: i
  }
}
function _a(e) {
  const a = e.data, t = {width: e.width + 2, height: e.height + 1},
        i = xa(t.width, t.height, 2);
  for (let r = 0; r < e.height; r++) {
    const l = Math.sin(Math.PI * (r + .5) / e.height);
    for (let n = 0; n < e.width; n++) {
      const f = 3 * (r * e.width + n);
      let u = a[f], p = a[f + 1], L = a[f + 2],
          d = .2126 * u + .7152 * p + .0722 * L;
      d *= l, i.set(n + 2, r, 0, i.get(n + 1, r, 0) + d / e.width),
          i.set(n + 1, r, 1, d)
    }
    const s = i.get(t.width - 1, r, 0);
    for (let n = 1; n < i.width; n++)
      i.set(n, r, 0, i.get(n, r, 0) / s), i.set(n, r, 1, i.get(n, r, 1) / s);
    i.set(0, r + 1, 0, i.get(0, r, 0) + s / e.height), i.set(0, r, 1, s)
  }
  const o = i.get(0, i.height - 1, 0);
  for (let r = 0; r < i.height; r++)
    i.set(0, r, 0, i.get(0, r, 0) / o), i.set(0, r, 1, i.get(0, r, 1) / o);
  return t.data = i.array, t
}
var va = {
  source: e => `
...
function Aa(e, a) {
  ...
  function s() {
    ...
    let n = t[o++];
    for (let f = 0; f < a; f++)
      r[f] = n % e + Math.random(), n = Math.floor(n / e);
    return r
  }
  return {
    next: s, restart: l, strataCount: e
  }
}
function qe(e, a) {
  const t = [];
  for (const l of a) t.push(Aa(e, l));
  const i = [];
  function o() {
    let l = 0;
    for (const s of t) {
      const n = s.next();
      for (const f of n) i[l++] = f
    }
    return i
  }
  function r() {
    for (const l of t) l.restart()
  }
  return {
    next: o, restart: r, strataCount: e
  }
}
let re = [], se = 0, Le = [];
function Ze(e, a) {
  const t = [], i = [];
  let o = 1;
  const r = {x: 1, y: 2, z: 3}, l = (s, n = 1) => {
    if (o = Math.max(n, o), s.primitives)
      for (let f = 0; f < s.primitives.length; f++) {
        const u = s.primitives[f];
        if (u.indices !== void 0) {
          const p = Le[u.meshID];
          t.push(
              u.indices[0] + p, u.indices[1] + p, u.indices[2] + p,
              s.primitives.length, u.faceNormal.x, u.faceNormal.y,
              u.faceNormal.z, u.meshID)
        } else
          t.push(re[u.meshID], u.materialID, u.meshID, 0, u.visible, .1, .1, 0);
        i.push(!1)
      }
    else {
      const f = s.bounds;
      t.push(
          f.min.x, f.min.y, f.min.z, r[s.splitAxis], f.max.x, f.max.y, f.max.z,
          null);
      const u = t.length - 1;
      i.push(!0), l(s.child0, n + 1), t[u] = t.length / 4 + a,
                                      l(s.child1, n + 1)
    }
  };
  return l(e), {
    maxDepth: o, count: t.length / 4, flatData: t, isBounds: i
  }
}
function ha() {
  re = [], se = 0, Le = []
...
function Sa(e, a, t) {
  ...
  for (let n = 0; n < e.length; n++) {
    const f = e[n], u = a[n].geometry;
    Le.push(r);
    const p = Ze(f, o);
    s = Math.max(p.maxDepth, s), re.push(o), o += p.count, i.push(p),
    r += u.getAttribute('position').count, l += p.flatData.length
  }
  return {
    totalBLASDataNum: l, totalBLASCount: o, flatBVHInfos: i, maxDepth: s
  }
}
function Ma(e, a) {
  const t = e.totalBLASDataNum, i = e.flatBVHInfos, o = a,
        r = o.flatData.length, l = t + r, s = new ArrayBuffer(4 * l),
        n = new Float32Array(s);
  for (let L = 0; L < i.length; L++) {
    const d = i[L], m = re[L] * 4, {flatData: x, isBounds: S} = d;
    for (let I = 0; I < S.length; I++) {
      let _ = 8 * I, h = _ + m;
      S[I] ? (n[h] = x[_], n[h + 1] = x[_ + 1], n[h + 2] = x[_ + 2],
              n[h + 3] = x[_ + 3]) :
             (n[h] = x[_], n[h + 1] = x[_ + 1], n[h + 2] = x[_ + 2],
              n[h + 3] = -x[_ + 3]),
          n[h + 4] = x[_ + 4], n[h + 5] = x[_ + 5], n[h + 6] = x[_ + 6],
                n[h + 7] = x[_ + 7]
    }
  }
  const {flatData: f, isBounds: u} = o, p = se * 4;
  for (let L = 0; L < u.length; L++) {
    let d = 8 * L, m = d + p;
    u[L], n[m] = f[d], n[m + 1] = f[d + 1], n[m + 2] = f[d + 2],
          n[m + 3] = f[d + 3], n[m + 4] = f[d + 4], n[m + 5] = f[d + 5],
          n[m + 6] = f[d + 6], n[m + 7] = f[d + 7]
  }
  return {
    bvhBufferDataNum: l, buffer: n
  }
}
function Fa(e, a, t, i) {
  const o = Sa(e, t);
  se = o.totalBLASCount;
  const r = je(a), {bvhBufferDataNum: l, buffer: s} = Ma(o, r);
  return {
    maxDepth: o.maxDepth + r.maxDepth, count: l / 4, buffer: s,
        tlasRootIndex: se
  }
}
let ee, me;
function Ga(e) {
  ...
  const t = new Uint32Array(a.count);
  for (let i = 0; i < t.length; i++) t[i] = i;
  return e.setIndex(new he(t, 1, !1)), e
}
function Ia(e) {
  let a = 0, t = 0;
  for (let o = 0; o < e.length; o++) {
    const r = e[o];
    r.geometry.isBufferGeometry ||
        (r.geometry = new wt().fromGeometry(r.geometry));
    const l = r.geometry;
    l.getIndex() || Ga(l),
        l.getAttribute('normal') ? l.normalizeNormals() :
                                   l.computeVertexNormals(),
        a += l.getAttribute('position').count, t += l.getIndex().count
  }
  const i = Yt(e);
  return {
    vertexTotalCount: a, indexCount: t, blasBVHs: i
  }
}
function Ke(e, a) {
  return Ht(e, a)
...
function Je(e) {
  const a = new Float32Array(e.length * 16);
  for (let t = 0; t < e.length; t++) {
    const i = e[t];
    i.updateMatrixWorld(!0);
    const o = i.matrixWorld.elements, r = 16;
    for (let l = 0; l < r; l++) a[t * r + l] = o[l]
  }
  return a
}
function Ta() {
...
function ga(e, a) {
  const t = Ke(e, a), i = je(t), o = Je(e), {flatData: r, isBounds: l} = i,
        {buffer: s} = ee, n = me * 4;
  for (let f = 0; f < l.length; f++) {
    let u = 8 * f, p = u + n;
    l[f], s[p] = r[u], s[p + 1] = r[u + 1], s[p + 2] = r[u + 2],
          s[p + 3] = r[u + 3], s[p + 4] = r[u + 4], s[p + 5] = r[u + 5],
          s[p + 6] = r[u + 6], s[p + 7] = r[u + 7]
  }
  return {
    bvhBufferInfo: ee, transformBufferData: o
  }
}
function Pa(e, a) {
  var u;
  let t = e;
  const {vertexTotalCount: i, blasBVHs: o} = Ia(e), r = Ke(t, a);
  ee = Fa(o, r, e), me = ee.tlasRootIndex;
  const l = new Float32Array(i * 4), s = new Float32Array(i * 4);
  let n = 0;
  for (let p = 0; p < e.length; p++) {
    const d = e[p].geometry, m = d.getAttribute('position').array,
          x = d.getAttribute('normal').array,
          S = (u = d.getAttribute('uv')) == null ? void 0 : u.array,
          I = d.getAttribute('position').count;
    for (let _ = 0; _ < I; _++)
      l[n + _ * 4 + 0] = m[_ * 3 + 0], l[n + _ * 4 + 1] = m[_ * 3 + 1],
                    l[n + _ * 4 + 2] = m[_ * 3 + 2],
                    l[n + _ * 4 + 3] = S ? S[_ * 2 + 0] : 0,
                    s[n + _ * 4 + 0] = x[_ * 3 + 0],
                    s[n + _ * 4 + 1] = x[_ * 3 + 1],
                    s[n + _ * 4 + 2] = x[_ * 3 + 2],
                    s[n + _ * 4 + 3] = S ? S[_ * 2 + 1] : 0;
    n += I * 4
  }
  const f = Je(e);
  return {
    vertexTotalCount: i, bvhBufferInfo: ee, positionBufferData: l,
        normalBufferData: s, transformBufferData: f, tlasRootIndex: me
  }
}
function Na(e, a) {
...
async function ya({
  ...
  s && s.onProgress && typeof s.onProgress == 'function' &&
 ...
 {
   uvTransformBufferTex: m,
   uvTransformBufferDataCount: x,
   wrappingRootIndex: S
 } = o,
 {
   vertexTotalCount: I,
   bvhBufferInfo: _,
   positionBufferData: h,
   normalBufferData: T,
   transformBufferData: v,
   tlasRootIndex: G
 } = await Na(L, d),
 P = _, F = $(t, {
          defines:
              Ee({
                OES_texture_float_linear: n,
                BVH_COLUMNS: ae(P.count).columnsLog,
                VERTEX_COLUMNS: ae(I).columnsLog,
                TRANSFORM_COLUMNS: ae(v.length / 4).columnsLog,
                UVTRANSFORM_COLUMNS: ae(x).columnsLog,
                SUPPORTED_MAPS: Me.length,
  ...
  return F.setTexture('diffuseMap', i.textures.diffuseMap),
         F.setTexture('normalMap', i.textures.normalMap),
         F.setTexture('pbrMap', i.textures.pbrMap),
         F.setTexture('pbrSGMap', i.textures.pbrSGMap),
         i.textures.emissiveMap &&
             F.setTexture('emissiveMap', i.textures.emissiveMap),
         F.setTexture('positionBuffer', j(t, h, 4)),
         F.setTexture('normalBuffer', j(t, T, 4)),
         F.setTexture('bvhBuffer', j(t, P.buffer, 4)),
         F.setUniform('tlasIndex', G),
         F.setTexture('transformBuffer', j(t, v, 4)),
         F.setUniform('wrappingIndex', S), F.setTexture('uvTransformBuffer', m),
         F
}
async function Ba(e, {
  bounces: a,
  ...
  S(a), I(t), _(t);
  function m(A, g) {
    L.setTexture('diffuseMap', A.textures.diffuseMap),
        L.setTexture('normalMap', A.textures.normalMap),
        L.setTexture('pbrMap', A.textures.pbrMap),
        L.setTexture('pbrSGMap', A.textures.pbrSGMap),
        A.textures.emissiveMap &&
        L.setTexture('emissiveMap', A.textures.emissiveMap);
    const {uvTransformBufferTex: b, wrappingRootIndex: R} = g;
    L.setUniform('wrappingIndex', R), L.setTexture('uvTransformBuffer', b)
  }
  function x(A) {
    const {meshes: g, materialIndexMap: b} = A, {
      bvhBufferInfo: R,
      transformBufferData: B
    } = ga(g, b),
                                        E = R.buffer;
    L.setTexture('transformBuffer', j(e, B, 4)),
        L.setTexture('bvhBuffer', j(e, E, 4))
  }
  function S(A) {
    d.length = 0, A = ie(A, 2, 8);
    for (let g = 1; g <= A; g++) d.push(2, 2, 2, 2), g >= 2 && d.push(1);
    L.setUniform('bounces', A), p && (p.strataCount = -1)
  }
  function I(A) {
    const {OES_texture_float_linear: g} = l,
                                     {environment: b, isTextureEnv: R} = A;
    if (R) {
      const B = ma(b);
      if (B) {
        const E = D(e, {
          data: B.data,
          storage: B.dataFormat,
          minFilter: g ? e.LINEAR : e.NEAREST,
          magFilter: g ? e.LINEAR : e.NEAREST,
          width: B.width,
          height: B.height
        });
        L.setTexture('envMap', E);
        const V = _a(B);
        L.setTexture('envMapDistribution', D(e, {
                       data: V.data,
                       storage: 'float',
                       width: V.width,
                       height: V.height
                     })),
            L.setUniform('envMapIntensity', s)
      }
    } else {
      const B = b.data;
      B && B.isColor ? L.setUniform('backgroundColor', [B.r, B.g, B.b]) :
                       L.setUniform('backgroundColor', [0, 0, 0])
    }
    T(n)
  }
  function _(A) {
    const {meshLights: g} = A;
    g &&
        (L.setUniform('lights.position[0]', g.position),
         L.setUniform('lights.emission[0]', g.emission),
         L.setUniform('lights.p1[0]', g.p1), L.setUniform('lights.p2[0]', g.p2),
         L.setUniform('lights.params[0]', g.params))
  }
  function h(A) {
    L.setUniform('envMapIntensity', A)
  }
  function T(A) {
    L.setUniform('enviromentVisible', Number(A))
  }
  function v(A) {
    L.setTexture(
        'noiseTex',
        D(e, {data: A, wrapS: e.REPEAT, wrapT: e.REPEAT, storage: 'halfFloat'}))
  }
  function G(A) {
    L.setUniform('frameCount', A)
  }
  function P() {
    L.setUniform('stratifiedSamples[0]', p.next())
  }
  function F(A, g) {
    L.setUniform('jitter', A, g)
  }
  function H(A) {
    A > 1 && A !== p.strataCount ? p = qe(A, d) : p.restart(),
                                   L.setUniform('strataSize', 1 / A), P()
  }
  function U(A, g) {
    L.setUniform('pixelSize', 1 / A, 1 / g)
  }
  function le(A) {
    L.setUniform('camera.transform', A.matrixWorld.elements),
        L.setUniform('camera.aspect', A.aspect),
        L.setUniform('camera.fov', .5 / Math.tan(.5 * Math.PI * A.fov / 180)),
        A.isLensCamera &&
        (L.setUniform('camera.aperture', A.aperture),
         L.setUniform('camera.focus', A.focus))
  }
  function fe({position: A}) {
    L.setTexture('gPosition', A)
  }
  function te() {
    L.bindTextures()
  }
  function N() {
    L.useProgram(!1), i.draw()
  }
  p = qe(1, d);
  function K() {
    Ta(), L.dispose()
  ...
  return {
    bindTextures: te, draw: N, outputLocs: L.outputLocs, textures: L.textures,
        setSize: U, setCamera: le, setGBuffers: fe, setNoise: v, setJitter: F,
        setFrameCount: G, setStrataCount: H, nextSeed: P, setEnvMapIntensity: h,
        setEnviromentVisible: T, updateBounces: S, updateEnvLight: I,
        updateMeshLight: _, updateMeshTransform: x, updateMeshMaterial: m,
        dispose: K
  }
}
var Ra = {
  source:
      'in vec3 aPosition;in vec3 aNormal;in vec2 aUv;uniform mat4 projView;uniform mat4 modelMat;uniform mat3 normalMat;out vec3 vPosition;out vec3 vNormal;out vec2 vUv;void main(){vec4 mPosition=modelMat*vec4(aPosition,1.);vPosition=mPosition.xyz;vNormal=normalize(normalMat*aNormal);vUv=aUv;gl_Position=projView*mPosition;}'
},
    Ua = {
      source: `
#define PI 3.14159265359
...
#endif
out_position=vec4(vPosition,float(meshIndex)+EPS);out_normal=vec4(normal,LGL_BS);out_color=color;}`
    };
function Ge(e, a, t) {
  if (a === void 0) return;
  const {itemSize: i, array: o} = t;
  if (e.enableVertexAttribArray(a),
      e.bindBuffer(e.ARRAY_BUFFER, e.createBuffer()),
      e.bufferData(e.ARRAY_BUFFER, o, e.STATIC_DRAW), o instanceof Float32Array)
    e.vertexAttribPointer(a, i, e.FLOAT, !1, 0, 0);
  else if (o instanceof Int32Array)
    e.vertexAttribIPointer(a, i, e.INT, 0, 0);
  else
    throw 'Unsupported buffer type'
}
function ba(e, a, t) {
  const i = t.getAttribute('position').count,
        o = new he(new Float32Array(3 * i), 3, !1),
        r = new he(new Float32Array(2 * i), 2, !1);
  Ge(e, a.attribLocs.aPosition, t.getAttribute('position')),
      Ge(e, a.attribLocs.aNormal, t.getAttribute('normal') || o),
      Ge(e, a.attribLocs.aUv, t.getAttribute('uv') || r),
      e.bindBuffer(e.ELEMENT_ARRAY_BUFFER, e.createBuffer()),
      e.bufferData(
          e.ELEMENT_ARRAY_BUFFER, new Uint32Array(t.getIndex().array),
          e.STATIC_DRAW)
}
function Ea(e, {materialBuffer: a, uvTransformBuffer: t, decomposedScene: i}) {
  const {
    meshes: o,
    materialIndexMap: r
  } = i,
 {uvTransformBufferTex: l, wrappingRootIndex: s} = t,
 n = $(e, {defines: a.defines, vertex: Ra, fragment: Ua});
  n.setTexture('diffuseMap', a.textures.diffuseMap),
      n.setTexture('normalMap', a.textures.normalMap),
      n.setUniform('wrappingIndex', s), n.setTexture('uvTransformBuffer', l);
  const f = [];
  for (let v = 0; v < o.length; v++) {
    const P = o[v].geometry, F = e.createVertexArray();
    e.bindVertexArray(F), ba(e, n, P), e.bindVertexArray(null), f.push(F)
  }
  let u = 0, p = 0;
  function L(v, G) {
    u = v, p = G
  ...
  let x = new we, S = new ze;
  function I() {
    x.copy(d.projectionMatrix), x.elements[8] += 2 * u, x.elements[9] += 2 * p,
        x.multiply(d.matrixWorldInverse), n.setUniform('projView', x.elements)
  }
  function _() {
    I(), e.enable(e.DEPTH_TEST), e.disable(e.CULL_FACE);
    for (let v = 0; v < o.length; v++) {
      const G = o[v];
      if (!G.visible) continue;
      const F = G.geometry.getIndex().count;
      n.setUniform('modelMat', G.matrixWorld.elements),
          S.getNormalMatrix(G.matrixWorld),
          n.setUniform('normalMat', S.elements), n.setUniform('meshIndex', v);
      const H = r.get(G.material);
      n.setUniform('materialIndex', H);
      const U = f[v];
      e.bindVertexArray(U), n.useProgram(),
          e.drawElements(e.TRIANGLES, F, e.UNSIGNED_INT, 0)
    }
    e.enable(e.CULL_FACE), e.disable(e.DEPTH_TEST)
  }
  function h(v, G) {
    n.setTexture('diffuseMap', v.textures.diffuseMap),
        n.setTexture('normalMap', v.textures.normalMap);
    const {uvTransformBufferTex: P, wrappingRootIndex: F} = G;
    n.setUniform('wrappingIndex', F), n.setTexture('uvTransformBuffer', P)
  }
  function T() {
    n.dispose()
  }
  return {
    draw: _, outputLocs: n.outputLocs, setCamera: m, setJitter: L,
        updateMeshMaterial: h, dispose: T
  }
}
var za = {
  source:
...
function Da(e, a) {
 ...
 r = new we;
  let l = .2, s = .2;
  function n(x) {
    r.multiplyMatrices(x.projectionMatrix, x.matrixWorldInverse),
        o.setUniform('historyCamera', r.elements)
  }
  function f(x, S) {
    o.setUniform('jitter', x, S)
  }
  function u(x) {
    l = x
  }
  ...
  function L(x) {
    o.setUniform('demodulateAlbedo', x)
  }
  function d(x) {
    const {
      light: S,
      ...
      previousLight: T,
      previousPosition: v,
      previousMomentLengthVariance: G
    } = x;
    o.setTexture('lightTex', S), o.setTexture('positionTex', I),
        o.setTexture('colorTex', _), o.setTexture('previousLightTex', T),
        o.setTexture('previousPositionTex', v),
        o.setTexture('previousColorTex', h),
        o.setTexture('previousMomentLengthVarianceTex', G),
        o.setUniform('colorBlendFactor', l),
        o.setUniform('momentBlendFactor', s), o.useProgram(), t.draw()
  }
  function m() {
    o.dispose()
  }
  return {
    draw: d, setJitter: f, setPreviousCamera: n, setDenoiseColorBlendFactor: u,
        setDenoiseMomentBlendFactor: p, setDemodulateAlbedo: L,
        getDenoiseFactors() {
      return {
        colorBlendFactor: l, momentBlendFactor: s
      }
    }
    , dispose: m
  }
...
function Ca(e, a) {
  ...
  const n = {gl: e, vertex: t.vertexShader, fragment: Va}, f = $(e, n), u = 3;
  let p = .5, L = .02, d = .35;
  function m({position: v, normal: G, color: P}) {
    f.setTexture('gPosition', v), f.setTexture('gNormal', G),
        f.setTexture('gColor', P)
  }
  function x(v) {
    p = v
  }
  ...
  function _(v) {
    f.setUniform('demodulateAlbedo', v)
  }
  function h(v) {
    let {light: G, reprojectData: P} = v;
    for (let F = 0; F < u; F++)
      f.setUniform('level', F), f.setUniform('colorFactor', 1 / (1 << F) * p),
          f.setUniform('normalFactor', 1 / (1 << F) * L),
          f.setUniform('positionFactor', 1 / (1 << F) * d),
          f.setUniform('stepwidth', (1 << F + 1) - 1),
          F === 0 ? f.setTexture('lightTex', G) :
                    f.setTexture('lightTex', i.color[0]),
          P ? (f.setUniform('useMomentVariance', 1),
               f.setTexture('reprojectDataTex', P)) :
              (f.setUniform('useMomentVariance', 0),
               f.setTexture('reprojectDataTex', null)),
          o.bind(), e.clear(e.COLOR_BUFFER_BIT),
          e.viewport(0, 0, e.drawingBufferWidth, e.drawingBufferHeight),
          f.useProgram(), t.draw(), o.unbind(), l();
    return i
  }
  function T() {
    f.dispose()
  ...
  return {
    draw: h, setGBuffers: m, setColorFactor: x, setNormalFactor: S,
        setPositionFactor: I, setDemodulateAlbedo: _, getDenoiseFactors() {
      return {
        colorFactor: p, normalFactor: L, positionFactor: d
      }
    }
    , setSize: s, dispose: T
  }
...
function Oa(e) {
  const a = e.getParameter(e.MAX_RENDERBUFFER_SIZE);
  if (a <= 8192) return 2e5;
  if (a === 16384) return 4e5;
  if (a >= 32768) return 6e5
}
function Qa(e) {
  const a = 21;
  let t = -1, i = 1, o = !1, r, l, s, n, f = 0, u = 0, p, L = Oa(e);
  function d() {
    const h = p / i, T = a - h;
    L += 5e3 * Math.sign(T) * Math.sqrt(Math.abs(T)), L = ie(L, 8192, f * u)
  }
  function m() {
    const h = f / u;
    o ? (r = Math.ceil(f / Math.sqrt(i)), l = Math.ceil(r / h),
         s = Math.ceil(f / r), n = Math.ceil(u / l), i = s * n) :
        (r = Math.ceil(f / Math.round(f / Math.sqrt(L * h))),
         l = Math.ceil(r / h), s = Math.ceil(f / r), n = Math.ceil(u / l),
         i = s * n)
  }
  function x(h, T) {
    f = h, u = T, S(), m()
  ...
  function I(h) {
    t++, p += h, t % i == 0 && (p && (d(), m()), p = 0, t = 0);
    const T = t === i - 1, v = t % s, G = Math.floor(t / s) % n;
    return {
      x: v * r, y: G * l, tileWidth: r, tileHeight: l, isFirstTile: t === 0,
          isLastTile: T
    }
  }
  function _(h) {
  ...
  return {
    nextTile: I, reset: S, setSize: x, setTileSlicesNumber: _
  }
}
var Ya =
...
function Ha(e) {
  const a = e.getParameter(e.MAX_RENDERBUFFER_SIZE);
  if (a <= 8192) return 8e4;
  if (a === 16384) return 15e4;
  if (a >= 32768) return 4e5
}
function Wa(e) {
  const a = 20;
  let t, i, o, r, l = new Se(1, 1), s = Ha(e);
  function n() {
    const p = t / i;
    o = Math.round(ie(Math.sqrt(s * p), 1, t)), r = Math.round(ie(o / p, 1, i)),
    l.set(o / t, r / i)
  }
  function f(p, L) {
    t = p, i = L, n()
  ...
  function u(p) {
    if (!p) return;
    const L = 600, d = a - p;
    s += L * d, s = ie(s, 8192, t * i), n()
  }
  return {
    adjustSize: u, setSize: f, scale: l, get width() {
      return o
    }
    , get height() {
      return r
...
async function ka({
  ...
  envMapIntensity: l,
  enviromentVisible: s,
  movingDownsampling: n,
  enableDenoise: f,
  enableTemporalDenoise: u,
  enableSpatialDenoise: p,
  useWorker: L,
  loadingCallback: d,
  fullSampleCallback: m
}) {
  const x = 20, S = 4, I = 6, _ = new Dt, h = Wa(e), T = Qa(e), v = ne(t, i),
        G = ke(e, v), P = Qe(e, v), F = Kt(e);
  let H = !1, U = 0, le, fe, te = !0;
  const N = await Ba(e, {
    bounces: r,
    decomposedScene: v,
    fullscreenQuad: F,
    ...
    loadingCallback: d
        ...
        K = Ea(
            e, {materialBuffer: G, uvTransformBuffer: P, decomposedScene: v}),
        A = Wt(e, {fullscreenQuad: F, toneMapping: o}),
        g = Xa(e, {fullscreenQuad: F}),
        b = Da(e, {fullscreenQuad: F, maxReprojectedSamples: x}),
        R = Ca(e, {fullscreenQuad: F, toneMapping: o}), B = new Image;
  B.src = Ya, B.onload = () => {
    N.setNoise(B), H = !0
  };
  let E, V, y, Q, X, W;
  function $e(c, M) {
    ...
    X = Y(), W = Y();
    const Z = D(e, {width: c, height: M, storage: 'halfFloat'}),
          Ae = D(e, {width: c, height: M, storage: 'float'}), ce = qt(e, c, M),
          Re = () => pe(e, {
            color:
                {0: D(e, {width: c, height: M, storage: 'float'}), 1: Z, 2: Ae},
            depth: ce
          });
    E = Re(), V = Re()
  }
  ...
  function et(c) {
    T.setTileSlicesNumber(c)
  }
  function tt() {
    const c = ne(t, i), M = ke(e, c), z = Qe(e, c);
    N.updateMeshMaterial(M, z), K.updateMeshMaterial(M, z)
  }
  function at() {
    const c = ne(t, i);
    N.updateMeshTransform(c)
  }
  function it(c) {
    N.updateBounces(c)
  }
  ...
  function rt(c) {
    N.setEnvMapIntensity(c)
  }
  function st(c) {
    N.setEnviromentVisible(c)
  }
  function lt(c) {
    A.setToneMapping(c)
  }
  ...
  function ct(c) {
    b.setDenoiseColorBlendFactor(c)
  }
  function pt(c) {
    b.setDenoiseMomentBlendFactor(c)
  }
  function Lt(c) {
    p = c, ue()
  }
  ...
  function _t(c) {
    R.setPositionFactor(c)
  }
  ue(!1);
  function ue(c = !0) {
    const M = Number(c && u && p);
    b.setDemodulateAlbedo(M), R.setDemodulateAlbedo(M)
  }
  function vt() {
    return Object.assign(R.getDenoiseFactors(), b.getDenoiseFactors())
  }
  let k = 0, q = 0;
  function At(c, M) {
    k = c, q = M, T.setSize(c, M), h.setSize(c, M), $e(c, M), R.setSize(c, M),
    A.setSize(c, M), g.setSize(c, M), te = !0
  }
  function xe(c, M) {
    N.setCamera(c), K.setCamera(c), b.setPreviousCamera(M), M.copy(c)
  }
  function _e(c, M, z = !0) {
    N.setSize(c, M), N.setFrameCount(U);
    const Y = z ? (Math.random() - .5) / c : 0,
          Z = z ? (Math.random() - .5) / M : 0;
    f || N.setJitter(Y, Z),
        U === 0 ? N.setStrataCount(1) :
                  U === S ? N.setStrataCount(I) : N.nextSeed()
  }
  function ht(c) {
    fe = c - le, le = c
  }
  function ge(c, M) {
    return kt(c.matrixWorld.elements, M.matrixWorld.elements) &&
        c.aspect === M.aspect && c.fov === M.fov
  }
  function J(c) {
    c.bind(), e.clear(e.COLOR_BUFFER_BIT), c.unbind()
  }
  function Pe(c, M, z) {
    c.bind(), e.blendEquation(e.FUNC_ADD), e.blendFunc(e.ONE, e.ONE),
        e.enable(e.BLEND), e.viewport(0, 0, M, z), N.draw(), e.disable(e.BLEND),
        c.unbind()
  }
  function St(c, M, z) {
    c.bind(), e.viewport(0, 0, M, z), N.draw(), c.unbind()
  }
  function de(c, M) {
    e.viewport(0, 0, e.drawingBufferWidth, e.drawingBufferHeight),
        A.draw({light: c, lightScale: M}), Q = c
  }
  function ve(c) {
    let M = A.draw({light: c}, !0);
    g.draw({light: M.color[0]})
  }
  function Ne() {
    E.bind(), e.clear(e.COLOR_BUFFER_BIT | e.DEPTH_BUFFER_BIT),
        e.viewport(0, 0, k, q), K.draw(), E.unbind(),
        R.setGBuffers(
            {position: E.color[0], normal: E.color[1], color: E.color[2]})
  }
  function Mt(c, M, z, Y, Z) {
    e.scissor(M, z, Y, Z), e.enable(e.SCISSOR_TEST), Pe(c, k, q),
        e.disable(e.SCISSOR_TEST)
  }
  function Ft() {
    U = 0, T.reset(), J(y), J(X), J(W)
  }
  function ye() {
    if (u &&
            (X.bind(), e.viewport(0, 0, k, q), b.draw({
              light: y.color[0],
              position: E.color[0],
              color: E.color[2],
              previousLight: Q,
              previousPosition: V.color[0],
              previousColor: V.color[2],
              previousMomentLengthVariance: W.color[1]
            }),
             X.unbind(), p || (ve(X.color[0]), Q = X.color[0])),
        p)
      if (u) {
        const c = R.draw({light: X.color[0], reprojectData: W.color[1]});
        ve(c.color[0]), Q = X.color[0]
      } else {
        const c = R.draw({light: y.color[0], reprojectData: null});
        ve(c.color[0]), Q = y.color[0]
      }
  }
  function Gt(c = !1) {
    const {
    ...
    Ae &&
        (U === 0 && (J(y), b.setPreviousCamera(_)), _e(k, q, !0),
         f && (u || p) && Ne(), N.bindTextures()),
        Mt(y, M, z, Y, Z), c && !ce && de(y.color[0]),
        ce &&
        (f && (u || p) ? (Q = y.color[0], ye(), Q = y.color[0]) :
                         de(y.color[0]),
         Ie(), Te(), U++, typeof m == 'function' && m())
  }
  function Be() {
    _e(h.width, h.height, !1), N.bindTextures(), St(y, h.width, h.height),
        de(y.color[0], h.scale), J(y)
  }
  function It(c) {
    !H || (ge(c, _) ? Gt() : (xe(c, _), te ? te = !1 : Be(), U = 0, T.reset()))
  }
  function Tt(c) {
    if (!!H) {
      if (Te(), Ie(), ge(c, _))
      ...
      xe(c, _), _e(k, q, !0), f && (u || p) && Ne(), N.bindTextures(),
          Pe(y, k, q), f && (u || p) ? ye() : de(y.color[0]),
          typeof m == 'function' && m()
    }
  }
  function gt() {
    N.dispose(), K.dispose(), A.dispose(), g.dispose(), b.dispose(),
        R.dispose(), E.dispose(), V.dispose(), y.dispose(), X.dispose(),
        W.dispose(), G.dispose(), P.dispose()
  }
  return {
    draw: It, fullDraw: Tt, setSize: At, time: ht, reset: Ft,
        getTotalSamplesRendered() {
      return U
    }
    , setTileSlicesNumber: et, updateBounces: it, updateEnvLight: nt,
        updateMeshLight: ot, setEnvMapIntensity: rt, setEnviromentVisible: st,
        setToneMapping: lt, setMovingDownsampling: ft, setDenoiseStatus: ut,
        setTemporalDenoiseStatus: dt, setDenoiseColorBlendFactor: ct,
        setDenoiseMomentBlendFactor: pt, setSpatialDenoiseStatus: Lt,
        setDenoiseColorFactor: mt, setDenoiseNormalFactor: xt,
        setDenoisePositionFactor: _t, getDenoiseFactors: vt,
        setDemodulateAlbedo: ue, updateMeshTransform: at,
        updateMeshMaterial: tt, dispose: gt
  }
}
class Ja {
  constructor(a = {}) {
    this.canvas = a.canvas || document.createElement('canvas'),
    this.gl = a.context || this.canvas.getContext('webgl2', {
      alpha: a.canvasAlpha || !1,
      depth: !0,
      stencil: !1,
      antialias: a.antialias || !1,
      powerPreference: 'high-performance',
      failIfMajorPerformanceCaveat: !0
    }),
    Fe(this.gl, Oe), this.optionalExtensions = Fe(this.gl, Zt),
    this._bounces = 2, this._envMapIntensity = 1, this._toneMapping = Vt,
    this._movingDownsampling = !1, this._enableDenoise = !1,
    this._enableTemporalDenoise = !0, this._enableSpatialDenoise = !0,
    this._enviromentVisible = !0, this.useTileRender = !1,
    this.renderWhenOffFocus = !0, this.useWorker = a.useWorker || !0,
    this.tileSlicesNum = 0,
    this.loadingCallback = a.loadingCallback ||
        {onProgress: t => console.log(t), onComplete: t => console.log(t)},
    this.fullSampleCallback = a.fullSampleCallback || null,
    this._isBuilding = !0, this.needsUpdate = !1,
    this.size = new Se(this.canvas.width, this.canvas.height),
    this.pixelRatio = 1, this.pipeline = null, this.currentTime = NaN,
    this.isValidTime = 1, this.lastFocus = !1, this.domElement = this.canvas
  }
  static isSupported() {
    const a = document.createElement('canvas').getContext(
        'webgl2', {failIfMajorPerformanceCaveat: !0});
    if (!a) return !1;
    const t = Fe(a, Oe);
    for (let i in t)
      if (!t[i]) return !1;
  ...
  async buildScene(a, t) {
    const {
      ...
      envMapIntensity: n,
      enviromentVisible: f,
      movingDownsampling: u,
      enableDenoise: p,
      enableTemporalDenoise: L,
      enableSpatialDenoise: d,
      useWorker: m,
      tileSlicesNum: x,
      loadingCallback: S,
      fullSampleCallback: I
    } = this;
    this._isBuilding = !0, a.updateMatrixWorld(), this.pipeline = await ka({
                                                    gl: i,
                                                    optionalExtensions: o,
                                                    scene: a,
                                                    camera: t,
    ...
    this.setSize(l.width, l.height), x && this.setTileSlicesNumber(x),
    this._isBuilding = !1,
    S && S.onComplete && typeof S.onComplete == 'function' &&
        S.onComplete('Complete!')
  }
  set bounces(a) {
    this._bounces = a, this.pipeline && this.pipeline.updateBounces(a)
  }
  get bounces() {
    return this._bounces
  }
  set envMapIntensity(a) {
    a = Number(a), this._envMapIntensity = a,
    this.pipeline && this.pipeline.setEnvMapIntensity(a)
  }
  get envMapIntensity() {
    return this._envMapIntensity
  }
  set toneMapping(a) {
    this._toneMapping = a, this.pipeline && this.pipeline.setToneMapping(a)
  }
  get toneMapping() {
    return this._toneMapping
  }
  set enviromentVisible(a) {
    this._enviromentVisible = a,
    this.pipeline && this.pipeline.setEnviromentVisible(a)
  }
  get enviromentVisible() {
    return this._enviromentVisible
  }
  set movingDownsampling(a) {
    a = !!a, this._movingDownsampling = a,
    this.pipeline && this.pipeline.setMovingDownsampling(a)
  }
  get movingDownsampling() {
    return this._movingDownsampling
  }
  set enableDenoise(a) {
    a = !!a, this._enableDenoise = a,
    this.pipeline && this.pipeline.setDenoiseStatus(a)
  }
  get enableDenoise() {
    return this._enableDenoise
  }
  set enableTemporalDenoise(a) {
    a = !!a, this._enableTemporalDenoise = a,
    this.pipeline && this.pipeline.setTemporalDenoiseStatus(a)
  }
  get enableTemporalDenoise() {
    return this._enableTemporalDenoise
  }
  set enableSpatialDenoise(a) {
    a = !!a, this._enableSpatialDenoise = a,
    this.pipeline && this.pipeline.setSpatialDenoiseStatus(a)
  }
  get enableSpatialDenoise() {
    return this._enableSpatialDenoise
  }
  setTileSlicesNumber(a) {
    a = Number(a), this.pipeline && this.pipeline.setTileSlicesNumber(a)
  }
  getContext() {
    return this.gl
  }
  updateMeshMaterial() {
    this.pipeline && this.pipeline.updateMeshMaterial()
  }
  updateMeshTransform() {
    this.pipeline && this.pipeline.updateMeshTransform()
  }
  updateEnvLight() {
  ...
  setDenoiseColorBlendFactor(a) {
    this.pipeline && this.pipeline.setDenoiseColorBlendFactor(a)
  }
  setDenoiseMomentBlendFactor(a) {
    this.pipeline && this.pipeline.setDenoiseMomentBlendFactor(a)
  }
  setDenoiseColorFactor(a) {
    this.pipeline && this.pipeline.setDenoiseColorFactor(a)
  }
  setDenoiseNormalFactor(a) {
    this.pipeline && this.pipeline.setDenoiseNormalFactor(a)
  }
  setDenoisePositionFactor(a) {
    this.pipeline && this.pipeline.setDenoisePositionFactor(a)
  }
  setDemodulateAlbedo(a) {
    this.pipeline && this.pipeline.setDemodulateAlbedo(a), this.needsUpdate = !0
  }
  getDenoiseFactors() {
    if (this.pipeline) return this.pipeline.getDenoiseFactors()
  }
  setSize(a, t, i = !0) {
    const {size: o, canvas: r, pipeline: l, pixelRatio: s} = this;
    o.set(a, t),
        r.width = o.width * s, r.height = o.height * s,
        i && (r.style.width = `${o.width}px`, r.style.height = `${o.height}px`),
        this.pipeline && l.setSize(o.width * s, o.height * s)
  }
  getSize(a) {
    const {size: t} = this;
    return a || (a = new Se), a.copy(t)
  }
  setPixelRatio(a) {
    const {size: t} = this;
    !a || (this.pixelRatio = a, this.setSize(t.width, t.height, !1))
  }
  getPixelRatio() {
  ...
  render(a, t) {
    if (!this._isBuilding) {
      if (!this.pipeline) {
        console.error(
            'Pipeline not initialized, The scene needs to be built first!');
        return
      }
      if (this.needsUpdate && (this.needsUpdate = !1, this.pipeline.reset()),
          !this.renderWhenOffFocus) {
        const i = document.hasFocus();
        if (i)
          i && !this.lastFocus && (this.lastFocus = i, this.restartTimer());
        else {
          this.lastFocus = i;
          return
        }
      ...
      this.isValidTime = 1, this.currentTime = NaN, t.updateMatrixWorld(),
      this.useTileRender ? this.pipeline.draw(t) : this.pipeline.fullDraw(t)
    }
  }
  dispose() {
    this.pipeline && this.pipeline.dispose(), this.pipeline = null
  }
}
export {Ja as L};
